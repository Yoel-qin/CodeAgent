"""代码审查 Agent（create_react_agent）集成测：自动工具调用循环 + 事件桥接 + 兜底降级。

与 test_bug_diagnosis.py / test_change_impact.py 同构（共享骨架 run_scenario_agent）：用一个可流的
假 ChatModel（先发 search_code tool_call、再发最终答案，作答轮经 run_manager.on_llm_new_token 推 token），
mock 检索层。验证：
  - retrieval（agent=CODE_REVIEW）→ agent_step（{tool,args,n}）+ citation → token 的事件序；
  - get_code_review_agent 异常 → _degrade 兜底仍产出 token。
"""
...
import app.agent.agents.code_review as cr
...
async def _run_review(state: dict, config: dict) -> list[dict]:
    g = StateGraph(AgentState)
    g.add_node("review", code_review)
...
async def test_code_review_loop_bridges_events(monkeypatch):
    monkeypatch.setattr(base, "configured", lambda: True)
    monkeypatch.setattr(cr, "get_chat_model", lambda: StepModel())
    monkeypatch.setattr(cr, "_agent", None)  # 强制用假模型重建 Agent
    ...
    events = await _run_review(
        {"query": "审查一下 A.m 的实现，有什么改进建议", "keywords": ["A", "m"], "rewritten": False},
    ...
    assert events[0]["data"]["agent"] == "CODE_REVIEW"    # 代码审查 Agent 标签
...
async def test_code_review_degrades_on_failure(monkeypatch):
    monkeypatch.setattr(base, "configured", lambda: True)

    def boom():
        raise RuntimeError("agent build failed")

    monkeypatch.setattr(cr, "get_code_review_agent", boom)
