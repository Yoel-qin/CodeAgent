...
Progress is well past the Phase-1 minimal loop (parse → chunk → index → 3-path recall → streaming LLM → SSE → chat UI). As of milestone **M41** the system also has: incremental sync + git rollback, multimodal docs (PDF/Word/txt + structured tables + OCR'd images), a knowledge-graph viewer, the full agent layer with cross-restart checkpointing + HITL, a **document self-healing arc** (detect stale doc↔code anchors → LLM-rewrite → human-approve → write back to PG/MinIO + re-embed + open a git PR), system monitoring, retrieval evaluation (CLI + API + UI), a ⌘K global search, a **pluggable domain-pack layer** (M36–M38) — the system is now a "generic base + domain packs" architecture (a **RocketMQ** first pack supplies trace templates / diagnosis trees / tuning rules / a config-key whitelist), and a **QA/幻觉 eval dimension** (M39) — a generic `LLMJudge` scores *system-generated answers* (the prior eval measured only retrieval recall) on faithfulness / answer-relevance / citation-accuracy / hallucination, plus an unverified-citation rate via the M34 `enforcer`. Newest addition: **可观测 Trace 结构化 + 全链路视图 (M41)** — a request-scoped `SpanCollector` (langgraph via `configurable["trace"]`, legacy via local wiring) records a structured span tree (request→intent→route→agent/collab→tool/llm/retrieval, with duration + real-usage tokens via `stream_options.include_usage` / langchain `usage_metadata`, chars/4 estimate fallback marked `estimated`) into `retrieval_logs.agent_steps` as a `{"version":2,...}` dict (three-shape compatible: null / old flat list / new dict — replay endpoint and agent-stats jsonpath predicates handle all three), exposed by `GET /v1/monitor/traces[/{log_id}]` and a new MonitorPage「全链路追溯」card (AntD Tree + inline-SVG waterfall, zero new deps; legacy rows get pseudo-spans). `docs/项目状态.md` (M1–M29) and `docs/重构开发清单.md` (the **M30+ refactor ledger**) are the authoritative per-milestone ledgers — trust them over the README.
