"""CostController：per-request 预算双闸（Plan 3 Task 8，自旧库 M42 ``app/agent/cost.py`` 移植精简）。

- token 闸：prompt+completion 合计（usage 真值优先；估算计入并标 estimated）
- 调用数闸：LLM 调用次数
- 超限语义：``record_*`` 只标记不抛（langchain 会吞回调异常，回调侧抛了也白抛）；
  ``check()`` 在显式调用点抛 BudgetExceeded。v2 的接缝在 :mod:`app.agent.react_base`——
  astream chunk 循环内轮询 ``cost.exceeded`` → raise → 降级出口（retrieve 兜底），请求永不中断。

与旧库差异：无 ``make_cost_controller`` 工厂/``cost_control_enabled`` 开关——v2 预算恒开，
每请求实例由 streaming 层按 ``settings.cost_max_tokens`` / ``settings.cost_max_llm_calls``
构造后放进 ``config["configurable"]["cost"]``（缺席时 react_base 兜底新建）。
"""
from __future__ import annotations


class BudgetExceeded(Exception):
    """预算超限。携带结构化信息供降级模板文案与 retrieval_meta.cost 记录。"""
...
class CostController:
    ...
    def __init__(self, *, max_tokens: int, max_llm_calls: int) -> None:
        ...
        self.spent_tokens = 0
        self.llm_calls = 0
        self.estimated = False            # 任一笔为估算则 True（诚实标记）
    ...
    def record_call(self) -> None:
        """一次 LLM 调用开始（on_chat_model_start）。超调用数闸只标记。"""
        self.llm_calls += 1
