"""测试生成 Agent（create_react_agent）集成测：自动工具调用循环 + 事件桥接 + 兜底降级。

与 test_bug_diagnosis.py / test_change_impact.py / test_code_review.py 同构（共享骨架
...
作答轮经 run_manager.on_llm_new_token 推 token），mock 检索层。验证：
  - retrieval（agent=TEST_GENERATION）→ agent_step（{tool,args,n}）+ citation → token 的事件序；
  - get_test_generation_agent 异常 → _degrade 兜底仍产出 token。
"""
...
import app.agent.agents.test_generation as tg
...
async def _run_test(state: dict, config: dict) -> list[dict]:
    g = StateGraph(AgentState)
    # 用 tg.test_generation（模块属性）而非直接 import 节点函数——后者会把以 test_ 开头的
    # 节点函数引入本测试模块命名空间，被 pytest 误当测试收集（fixture 'state' not found）。
    g.add_node("test", tg.test_generation)
...
async def test_test_generation_loop_bridges_events(monkeypatch):
    monkeypatch.setattr(base, "configured", lambda: True)
    monkeypatch.setattr(tg, "get_chat_model", lambda: StepModel())
    monkeypatch.setattr(tg, "_agent", None)  # 强制用假模型重建 Agent
    ...
    events = await _run_test(
        {"query": "帮我给 A.m 写单元测试", "keywords": ["A", "m"], "rewritten": False},
    ...
    assert events[0]["data"]["agent"] == "TEST_GENERATION"  # 测试生成 Agent 标签
    ...
    assert "".join(e["data"]["content"] for e in events if e["event"] == "token") == "TEST"


# ---- 兜底降级：Agent 构建失败 → _degrade 仍作答 ----


async def test_test_generation_degrades_on_failure(monkeypatch):
    monkeypatch.setattr(base, "configured", lambda: True)

    def boom():
        raise RuntimeError("agent build failed")

    monkeypatch.setattr(tg, "get_test_generation_agent", boom)
