"""纯文本解析（Phase 1.5a）：按空行分段为 PARAGRAPH DocElement。

``decode_text`` 为 UTF-8→gb18030→latin-1 解码阶梯（中文 Windows .txt 常为 GBK/GB18030，
纯 UTF-8 解码会乱码）；放此叶子模块供 router 复用，避免 router↔parser 循环导入。
"""
