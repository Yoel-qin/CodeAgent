...
def _ocr_placeholder(data: bytes) -> str:
    """v1 不做 OCR，返回空字符串。后续接入 OCR 引擎时替换此函数。"""
    return ""
...
def parse_pdf(data: bytes, file_path: str) -> tuple[list[DocElement], ParseMeta]:
    ...
    try:
        ...
        for pno, page in enumerate(doc):
            ...
            # 表格（PyMuPDF find_tables，best-effort）
            try:
            ...
            # 图片（PyMuPDF extract_image，OCR 占位）
            try:
