...
Progress is well past the Phase-1 minimal loop (parse → chunk → index → 3-path recall → streaming LLM → SSE → chat UI). As of milestone **M43** the system also has: incremental sync + git rollback, multimodal docs (PDF/Word/txt + structured tables + OCR'd images), a knowledge-graph viewer, the full agent layer with cross-restart checkpointing + HITL, a **document self-healing arc** (detect stale doc↔code anchors → LLM-rewrite → human-approve → write back to PG/MinIO + re-embed + open a git PR), system monitoring, retrieval evaluation (CLI + API + UI), a ⌘K global search, a **pluggable domain-pack layer** (M36–M38) — the system is now a "generic base + domain packs" architecture (a **RocketMQ** first pack supplies trace templates / diagnosis trees / tuning rules / a config-key whitelist), and a **QA/幻觉 eval dimension** (M39) — a generic `LLMJudge` scores *system-generated answers* (the prior eval measured only retrieval recall) on faithfulness / answer-relevance / citation-accuracy / hallucination, plus an unverified-citation rate via the M34 `enforcer`. Then **可观测 Trace 结构化 + 全链路视图 (M41)** — a request-scoped `SpanCollector` (langgraph via `configurable["trace"]`, legacy via local wiring) records a structured span tree (request→intent→route→agent/collab→tool/llm/retrieval, with duration + real-usage tokens via `stream_options.include_usage` / langchain `usage_metadata`, chars/4 estimate fallback marked `estimated`) into `retrieval_logs.agent_steps` as a `{"version":2,...}` dict (three-shape compatible: null / old flat list / new dict — replay endpoint and agent-stats jsonpath predicates handle all three), exposed by `GET /v1/monitor/traces[/{log_id}]` and a new MonitorPage「全链路追溯」card (AntD Tree + inline-SVG waterfall, zero new deps; legacy rows get pseudo-spans). Newest addition: **CostController + Redis 缓存 (M42)** — opt-in per-request budget (`agent/cost.py`: real-usage token counting + LLM-call cap, over-budget → template degrade + `retrieval_meta.cost`) plus a soft-fail Redis cache layer (exact-match QA cache replaying the identical SSE contract on both engines + query-embedding cache; Redis down = plain miss). Newest addition: **Feedback loop (M43)** — rich NOT_HELPFUL feedback with 6-category enum + free-text correction, high-signal entries auto-promoted into `candidate_eval_queries` (uk `source_message_id` idempotent) for future eval-set seeding, plus a 5-segment analytics report (`GET /v1/monitor/feedback-report`: summary / categories / by_repo / keywords / hallucination_alerts cross-referencing M34 enforcement `ratio`). `docs/项目状态.md` (M1–M29) and `docs/重构开发清单.md` (the **M30+ refactor ledger**) are the authoritative per-milestone ledgers — trust them over the README.
...
- `COST_CONTROL_ENABLED` (default off) + `COST_MAX_TOKENS_PER_REQUEST` (80000) + `COST_MAX_LLM_CALLS` (12) — M42 per-request budget: `agent/cost.py` `CostController` (per-request, injected via `configurable["cost"]`, never into checkpointed state) counts real usage tokens via the `CostCallbackHandler` mounted on all LLM call sites (`_base`/`query_analysis`/`collab`; chars/4 estimate fallback when usage is absent), polled in the chunk loop + each collab round + checked before generate. Over-budget → `BudgetExceeded` → template degrade notice (request never breaks) + spend recorded into `retrieval_meta.cost`. Off = no controller is created at all.
...
| Services | `services/` | Orchestration: `chat_service`, plus `sync`/`document`/`graph`/`monitor`/`search`/`eval_run`/`staleness_sweep`/`sweep_rewrite`/`doc_maintenance`/`doc_pr`/`maintenance`/`agent_stats`/`feedback` |
...
**Companion REST endpoints** (also under `/v1/chat`, in `conversations.py`): `GET /conversations` + `/conversations/{id}` (list / detail-with-messages), `GET /messages/{id}/retrieval` (replays the persisted funnel as stage1/stage2/stage3), `POST /suggestions` (LLM follow-up questions), `POST /messages/{id}/feedback` (`HELPFUL`/`NOT_HELPFUL`, written onto the `RetrievalLog` to collect Phase 8 LTR data; **M43**: `NOT_HELPFUL` accepts optional `categories` — 6-category Chinese-literal enum — + `correction` (<=2000 chars); high-signal feedback (categories include 答案错误/内容编造 or non-empty correction) auto-inserts into `candidate_eval_queries` with uk `source_message_id` idempotency; response includes `candidate_created` flag; `HELPFUL` with categories -> 422; old binary body remains backward-compatible).
...
**Observability**: every tool call → `agent_step` SSE event, accumulated in `streaming.stream_graph`, persisted as `retrieval_logs.agent_steps` (JSONB; migration `b7e2d09af3c1`); M41 upgrades it to a structured span tree (`{"version":2,"spans":[...]}` dict with duration/token/parent — tools land as `kind="tool"` spans, and `_build_agent_trace` maps them back to `{tool,args,n}` so the drawer is unchanged), and replayed as the `agent` segment of `GET /v1/chat/messages/{id}/retrieval`. The trace is keyed on `agent_steps` being non-empty (not on `meta.mode=="agent"`), so steps taken before a degrade are still shown. The same column feeds the `/v1/agents/stats` KPIs (`services/agent_stats_service.py`).
...
- **Monitor** (`api/v1/monitor.py` + `services/monitor_service.py`): 7 GET endpoints — `retrieval-perf` (latency p50/p95 + funnel means + rerank rate + feedback, raw `percentile_cont` over `retrieval_logs`), `api-usage` (**PG-derived proxy** — there's no client-side usage telemetry, so LLM calls ≈ assistant messages, embedding ≈ retrieval_logs rows, tokens ≈ chars/4; the response `note` states this), `index-stats` (PG/Milvus/ES counts), `resources` (connectivity + storage size per component), `traces` (M41 全链路追溯：span 树列表 + legacy 伪 span 合成 + has_trace 过滤), `traces/{log_id}` (span 树详情 + 瀑布图数据), `feedback-report` (M43 五段式反馈报告：summary / categories / by_repo / keywords / hallucination_alerts — 幻觉告警与 M34 enforcement `ratio` 交叉; 每段独立降级). Each component is independently try/except'd → never 500s. Frontend: `pages/MonitorPage.tsx`.
...
- **Pages**: `ChatPage` (sidebar + `CitationCard` + `RetrievalDrawer` rendering the stage1/2/3 funnel **and** the `agent` tool trace), `DocumentsPage` (upload→parse→table preview), `GraphPage` (`react-cytoscapejs` viz), `SyncPage`, `AgentsPage` (KPIs from `/v1/agents/stats`), `StalenessPage` (report + approval queue + rewrite-preview drawer), `MonitorPage`（M41 增第 5 卡「全链路追溯」: TraceView 树 + 内联 SVG 瀑布图; M43 增第 6 卡「反馈闭环」: 五段式反馈报告 + 候选 eval 查询列表）, `EvalPage` (3 Tabs: 单次评测 / A/B 消融 / **LLM 评判 QA·幻觉**; KPIs + history + inline-SVG `Sparkline` trend — **there is no chart library in the repo**).
