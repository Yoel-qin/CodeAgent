"""变更影响 Agent（create_react_agent）集成测：自动工具调用循环 + 事件桥接 + 兜底降级。

与 test_doc_answer.py / test_code_understand.py 同构（M4 共享骨架 run_scenario_agent）：
用一个可流的假 ChatModel（先发 search_code tool_call、再发最终答案，作答轮经
run_manager.on_llm_new_token 推 token），mock 检索层。验证：
  - retrieval（agent=CHANGE_IMPACT）→ agent_step + citation（工具桥接）→ token 的事件序；
  - get_change_impact_agent 异常 → _degrade 兜底仍产出 token。
"""
...
import app.agent.agents.change_impact as ci
...
async def _run_ci(state: dict, config: dict) -> list[dict]:
    g = StateGraph(AgentState)
    g.add_node("ci", change_impact)
    g.add_edge(START, "ci")
    g.add_edge("ci", END)
    events: list[dict] = []
...
async def test_change_impact_loop_bridges_events(monkeypatch):
    monkeypatch.setattr(base, "configured", lambda: True)
    monkeypatch.setattr(ci, "get_chat_model", lambda: StepModel())
    monkeypatch.setattr(ci, "_agent", None)  # 强制用假模型重建 Agent
    ...
    events = await _run_ci(
        {"query": "修改 A.m 会影响哪些地方", "keywords": ["A", "m"], "rewritten": False},
        {"configurable": {"thread_id": "t", "session": object(), "top_k": 8,
                          "agent_type": "CHANGE_IMPACT"}},
    )
    seq = [e["event"] for e in events]
    assert seq[0] == "retrieval"                            # _emit_retrieval_meta 前置
    assert events[0]["data"]["agent"] == "CHANGE_IMPACT"    # 变更影响 Agent 标签
...
async def test_change_impact_degrades_on_failure(monkeypatch):
    monkeypatch.setattr(base, "configured", lambda: True)

    def boom():
        raise RuntimeError("agent build failed")

    monkeypatch.setattr(ci, "get_change_impact_agent", boom)
